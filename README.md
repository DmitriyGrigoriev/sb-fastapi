...
fastapi-mvc available under: http://sb-fastapi.192.168.49.2.nip.io/
...
...
Kubernetes cluster ready
FastAPI available under: http://sb-fastapi.192.168.49.2.nip.io/
You can delete dev-env by issuing: make clean
vagrant@ubuntu-focal:/syncd$ kubectl get all -n sb-fastapi
NAME                                                     READY   STATUS    RESTARTS   AGE
pod/sb-fastapi-7f4dd8dc7f-p2kr7                1/1     Running   0          55s
...
vagrant@ubuntu-focal:/syncd$ curl http://sb-fastapi.192.168.49.2.nip.io/api/ready
{"status":"ok"}
```
...
To bootstrap local minikube Kubernetes cluster exposing `sb-fastapi` application:
```shell
make dev-env
```
...
```shell
$ sb-fastapi --help
Usage: sb-fastapi [OPTIONS] COMMAND [ARGS]...
...
Commands:
  serve  sb-fastapi CLI serve command.
$ sb-fastapi serve --help
Usage: sb-fastapi serve [OPTIONS]

  sb-fastapi CLI serve command.
...
```shell
$ make image
$ podman run -dit --name sb-fastapi -p 8000:8000 sb-fastapi:$(cat TAG)
f41e5fa7ffd512aea8f1aad1c12157bf1e66f961aeb707f51993e9ac343f7a4b
$ podman ps
CONTAINER ID  IMAGE                                 COMMAND               CREATED        STATUS            PORTS                   NAMES
f41e5fa7ffd5  localhost/sb-fastapi:0.1.0  /usr/bin/fastapi ...  2 seconds ago  Up 3 seconds ago  0.0.0.0:8000->8000/tcp  sb-fastapi
$ curl localhost:8000/api/ready
{"status":"ok"}
```
...
All application configuration is available in `sb_backend.config` submodule.
...
| FASTAPI_PROJECT_NAME | `"sb-fastapi"`                                                  | FastAPI project name.                                          |
...
1. Source: `sb_backend.config/gunicorn.conf.py`
2. [Gunicorn configuration file documentation](https://docs.gunicorn.org/en/latest/settings.html)

### Routes definition

Endpoints are defined in `sb_backend.config.router`. Just simply import your controller and include it to FastAPI router:
...
You can implement your own web routes logic straight away in `sb_backend.app.controllers.api.v1` submodule. For more information please see [FastAPI documentation](https://fastapi.tiangolo.com/tutorial/).

### Utilities

For your discretion, I've provided some basic utilities:
* RedisClient `sb_backend.app.utils.redis`
* AiohttpClient `sb_backend.app.utils.aiohttp_client`
...
Example:
```python
from sb_backend.app.utils import RedisClient

response = RedisClient.get("Key")
```
```python
from sb_backend.app.utils import AiohttpClient
...
Source: `sb_backend.app.exceptions.http.py`
