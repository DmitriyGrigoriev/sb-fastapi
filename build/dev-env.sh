...
minikube image load "sb-fastapi:${TAG}"

echo "[dev-env] creating sb-fastapi namespace"
kubectl create namespace sb-fastapi

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl get namespace sb-fastapi -n sb-fastapi"
...
kubectl create -f manifests/persistent-storage-no-pvc-deletion.yaml -n sb-fastapi

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment.apps/rfs-redisfailover-persistent-keep -n sb-fastapi"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
STATEFULSET_REPLICAS=$(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.spec.replicas}' -n sb-fastapi)
ATTEMPTS=0
until [[ ${STATEFULSET_REPLICAS} -eq $(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.status.readyReplicas}' -n sb-fastapi) ]] || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done

echo "[dev-env] installing sb-fastapi charts"
helm upgrade --install \
    sb-fastapi charts/sb-fastapi \
    --namespace sb-fastapi \
    --set ingress.host.name="sb-fastapi.${INGRESS_HOST}"

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment/sb-fastapi -n sb-fastapi"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
FastAPI available under: http://sb-fastapi.${INGRESS_HOST}/
