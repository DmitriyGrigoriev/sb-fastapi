...
Now you should access CLI script: $ sb-fastapi --help
Alternatively you can access CLI script via poetry run: $ poetry run sb-fastapi --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_SB_BACKEND_COMPLETE=source_bash sb-fastapi)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_SB_BACKEND_COMPLETE=source_zsh sb-fastapi)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_SB_BACKEND_COMPLETE=source_fish sb-fastapi)' >> ~/.config/fish/completions/sb-fastapi.fish
EOF